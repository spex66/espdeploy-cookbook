...
# xxx pa: sort with named arguments key / reverse
...
#                cmp = __builtins__.cmp
                ...
                print '??', type(__builtins__), repr(__builtins__.keys())
                raise
        seq.sort(lambda x,y: cmp(key(x), key(y)))
    else:
        if cmp == None:
            seq.sort()
        else:
...
## xxx pa: overload list.sort
# def _sort(self, cmp=None, key=None, reverse=False):
    # sort(self, cmp, key, reverse)
# list.sort = _sort # overload?
# Traceback (most recent call last):
  # File "<stdin>", line 1, in ?
  # File "C:\arwa\dev\sitepackages\utils.py", line 89, in ?
    # list.sort = _sort # overoad?
# TypeError: can't set attributes of built-in/extension type 'list'# xxx pa: noway
